...
for ligne in fic:
	...
	if nb_lignes > 1 and nb_lignes < 100: 
...
#test et ajout dans le dictionnaire
		for processus in liste_processus:	
			...
			if proteine_famille1 not in GO_processus_biologiques[processus]:
				GO_processus_biologiques[processus][proteine_famille1]=[proteine_nom1]

			elif proteine_famille1 in GO_processus_biologiques[processus]:
				GO_processus_biologiques[processus][proteine_famille1].append(proteine_nom1)
"""
...
res = open("resultat_GO_familles", "w")

res.write("parent\tenfant\tlien de parenté\n")
...
for i in GO_processus_biologiques.keys():
	nb_GO +=1
	if nb_GO < 100:
		for j in GO_processus_biologiques[i].keys():
			res.write(i + "\t" + j + "\tlien biologique\n" )
...
for i in GO_processus_biologiques.keys():
	...
	if nb_GO < 100:
		for j in GO_processus_biologiques[i].keys():
			for k in GO_processus_biologiques[i][j]:
				res.write(j + "\t" + k + "\tlien familiale\n" )
